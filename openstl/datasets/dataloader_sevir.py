...
class SEVIRDataset(Dataset):
    ...
    """
    def __init__(self,
                 data_root='/home/bingxing2/ailab/group/ai4multi/data_full/sevir/vil_processed',
                 data_name='vil',
                 is_train=True,
                 use_augment=False,
        ...
        if is_train:
            self._hdf_file = '/home/bingxing2/ailab/group/ai4multi/data_full/sevir/vil_processed/vil_training.h5'
        else:
            self._hdf_file = '/home/bingxing2/ailab/group/ai4multi/data_full/sevir/vil_processed/vil_testing.h5'
    ...
    def __getitem__(self, idx):
        ...
        if X_max > X_min:
            ...
            X = torch.zeros_like(X)  # 如果最大值和最小值相等，设置为 0

        if Y_max > Y_min:
            Y = (Y - Y_min) / (Y_max - Y_min)
        else:
            Y = torch.zeros_like(Y)  # 如果最大值和最小值相等，设置为 0

        # 改变维度顺序以适应模型输入格式 (可选，取决于模型的输入需求)
    ...
    #     # Expand single-channel input to 3 channels if necessary
    ...
    #     # if X.shape[1] == 1:
    #     #     X = X.repeat(1, 3, 1, 1)
    #     # print(f"Expanded X shape (after channel repeat if necessary): {X.shape}")        
    #     return X, Y
...
if __name__ == '__main__':
    path = '/home/bingxing2/ailab/group/ai4multi/data_full/sevir'
