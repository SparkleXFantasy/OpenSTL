...
class EarthFormerConfig:
    def __init__(self, initial_shapes, input_shape, initial_downsample_scales):
        ...
        self.input_shape = input_shape    # hidden size for backbone
        self.initial_downsample_scales = initial_downsample_scales
        base_units = 64
        block_units = None
        scale_alpha = 1.0
        ...
        initial_downsample_scale = [1, 1, 1]
        ...
        self.model_kwargs = dict(
            ...
            input_shape=input_shape,    # (T, H, W, C)
            target_shape=input_shape,   # (T, H, W, C)
...
class MultiEarthFormer_Model(nn.Module):
    """
        EarthFormer for multi encoders and decoders.
    """
    def __init__(self, enc_dec_configs, **kwargs):
        ...
        for enc_dec_config in enc_dec_configs:
            assert(enc_dec_config['input_shape'][:3] == input_shape[:3])
