...
class Upsample(nn.Upsample):
    ...
    def forward(self, input: Tensor) -> Tensor:
        if input.dtype == torch.bfloat16:
            return super().forward(input.to(torch.float32)).to(torch.bfloat16)
        else:
...
class Upsample3DLayer(nn.Module):
    ...
    def __init__(self,
        ...
        if temporal_upsample:
            self.up = Upsample(size=target_size, mode='nearest')  # 3D upsampling
        else:
            self.up = Upsample(size=(target_size[1], target_size[2]), mode='nearest')  # 2D upsampling
...
CuboidSelfAttentionPatterns = Registry('CuboidSelfAttentionPatterns')
...
# basic patterns
CuboidSelfAttentionPatterns.register_module(name='full', module=full_attention)
CuboidSelfAttentionPatterns.register_module(name='axial', module=self_axial)
CuboidSelfAttentionPatterns.register_module(name='video_swin', module=self_video_swin)
CuboidSelfAttentionPatterns.register_module(name='divided_st', module=self_divided_space_time)
...
for p in [1, 2, 4, 8, 10]:
    for m in [1, 2, 4, 8, 16, 32]:
        CuboidSelfAttentionPatterns.register_module(
            name=f'video_swin_{p}x{m}', module=functools.partial(self_video_swin, P=p, M=m)
        )
...
CuboidSelfAttentionPatterns.register_module(name='spatial_lg_v1', module=self_spatial_lg_v1)
# CuboidSelfAttentionPatterns.register('spatial_lg_v1', self_spatial_lg_v1)
# spatial_lg
for m in [1, 2, 4, 8, 16, 32]:
    CuboidSelfAttentionPatterns.register_module(
        name=f'spatial_lg_{m}', module=functools.partial(self_spatial_lg_v1, M=m)
    )
    # CuboidSelfAttentionPatterns.register(
    #     f'spatial_lg_{m}',
...
for k in [2, 4, 8]:
    CuboidSelfAttentionPatterns.register_module(
        name=f'axial_space_dilate_{k}', module=functools.partial(self_axial_space_dilate_K, K=k)
    )
    # CuboidSelfAttentionPatterns.register(
...
for k in [1, 2, 4, 8]:
    CuboidCrossAttentionPatterns.register_module(
        name=f'cross_{k}x{k}', module=functools.partial(cross_KxK, K=k)
    )
    CuboidCrossAttentionPatterns.register_module(
        name=f'cross_{k}x{k}_lg', module=functools.partial(cross_KxK_lg, K=k)
    )
    CuboidCrossAttentionPatterns.register_module(
        name=f'cross_{k}x{k}_heter', module=functools.partial(cross_KxK_heter, K=k)
    )
...
class CuboidTransformerModel(nn.Module):
    ...
    def __init__(self,
                ...
                initial_downsample_scales=None,    # the downsample scales for each encoder/decoder in order
        ...
        if initial_downsample_scales is None:
            ...
            new_input_shape = self.get_multi_initial_encoder_final_decoders(
                initial_shapes=initial_shapes,
                initial_downsample_scales=initial_downsample_scales,
                initial_downsample_type=initial_downsample_type,
                activation=initial_downsample_activation,
                # initial_downsample_type=="conv"
                initial_downsample_conv_layers=initial_downsample_conv_layers,
    ...
    def get_multi_initial_encoder_final_decoders(
            self,
            initial_shapes,
            initial_downsample_type,
            activation,
        ...
        new_input_shapes = []
        new_output_shapes = []
        for initial_shape, initial_downsample_scale in zip(initial_shapes, initial_downsample_scales):
            if self.initial_downsample_type == "conv":
                if isinstance(initial_downsample_scale, int):
                    initial_downsample_scale = (1, initial_downsample_scale, initial_downsample_scale)
                elif len(initial_downsample_scale) == 2:
                ...
                T_in, H_in, W_in, C_in = initial_shape
                # if any(ele > 1 for ele in initial_downsample_scale):
                self.initial_encoders.append(InitialEncoder(dim=C_in,
                                                    out_dim=self.base_units,
                                                    downsample_scale=initial_downsample_scale,
                                                    num_conv_layers=initial_downsample_conv_layers,
                                                    padding_type=padding_type,
                ...
                new_output_shapes.append(initial_shape)
                T_out, H_out, W_out, C_out = initial_shape
                self.final_decoders.append(FinalDecoder(dim=self.base_units,
                                                target_thw=(T_out, H_out, W_out),
                                                num_conv_layers=final_upsample_conv_layers,
                                                activation=activation,
                                                conv_init_mode=self.conv_init_mode,
                                                linear_init_mode=self.down_up_linear_init_mode,
                                                norm_init_mode=self.norm_init_mode))
                new_input_shapes.append(self.initial_encoders[-1].patch_merge.get_out_shape(initial_shape))
                self.dec_final_projs.append(nn.Linear(self.base_units, C_out))
                # else:
                #     self.initial_encoder = nn.Linear(C_in, self.base_units)
                #     self.final_decoder = nn.Identity()
                #     self.dec_final_proj = nn.Linear(self.base_units, C_out)
                ...
                self.final_decoder = FinalStackUpsamplingDecoder(
                    ...
                    norm_init_mode=self.norm_init_mode)
                self.dec_final_proj = nn.Linear(dec_target_shape_list[-1][-1], C_out)
                new_input_shape = self.initial_encoders[-1].get_out_shape_list(self.input_shape)[-1]
            else:
                raise NotImplementedError
            # self.input_shape_after_initial_downsample = new_input_shape
            # T_in, H_in, W_in, _ = new_input_shape
        self.initial_encoders = nn.ModuleList(self.initial_encoders)
        self.final_decoders = nn.ModuleList(self.final_decoders)
        self.dec_final_projs = nn.ModuleList(self.dec_final_projs)
        for x in new_input_shapes:
            print(x)
            assert x[0] == new_input_shapes[0][0] and x[1] == new_input_shapes[0][1] and x[2] == new_input_shapes[0][2] 
        return new_input_shapes[0]

    def reset_parameters(self):
        if self.num_global_vectors > 0:
            nn.init.trunc_normal_(self.init_global_vectors, std=.02)
        if len(self.initial_encoders) != 0:
            for encoder in self.initial_encoders:
                encoder.reset_parameters()
            for decoder in self.final_decoders:
                decoder.reset_parameters()
            for dec_final_proj in self.dec_final_projs:
                apply_initialization(dec_final_proj,
    ...
    def forward(self, x, data_cls_idx=None, verbose=False):
        """
        ...
        if data_cls_idx is None:
            x = self.initial_encoder(x)
            if verbose:
                print(f'Shape After initial encoder:{x.shape}')
            ...
            if verbose:
                print(f'Shape After decoder:{dec_out.shape}')
            dec_out = self.final_decoder(dec_out)
            if verbose:
                print(f'Shape After final_decoder:{dec_out.shape}')
            out = self.dec_final_proj(dec_out)
            if verbose:
                print(f'out shape:{out.shape}')
            return out
        else:
            data_cls_idx = int(data_cls_idx)
            initial_encoder = self.initial_encoders[data_cls_idx]
            final_decoder = self.final_decoders[data_cls_idx]
            dec_final_proj = self.dec_final_projs[data_cls_idx]
            x = initial_encoder(x)
            if verbose:
                print(f'Shape After initial encoder:{x.shape}')
            x = self.enc_pos_embed(x)
            ...
            if verbose:
                print(f'Shape After decoder:{dec_out.shape}')
            dec_out = final_decoder(dec_out)
            if verbose:
                print(f'Shape After final_decoder:{dec_out.shape}')
            out = dec_final_proj(dec_out)
            if verbose:
                print(f'out shape:{out.shape}')
            return out